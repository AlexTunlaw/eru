...
# ---------------------------------------------------------------------------
# Usage examples:
#   compute_tri_word_hash("what does this do") --> "begin south grass"
#   compute_tri_word_hash(123) --> "clever man both"
#   compute_tri_word_hash(True) --> "blood radio pour"
#
# Good for making various IDs easy to remember or scan by an eye.
...
tokens = text_source.translate(str.maketrans({",": " ", ".": " ", "\n": " "})).split(" ")
tokens = [token for token in tokens if token]

# ---------------------------------------------------------------------------

def compute_tri_word_hash(s):
    h1 = FnvHash.compute(s)
    h2 = FnvHash.compute(s, seed=h1)
    h3 = FnvHash.compute(s, seed=h2)
    l = len(tokens)
    return " ".join([tokens[i] for i in [h1 % l, h2 % l, h3 % l]])
